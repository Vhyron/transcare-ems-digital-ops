...
export async function POST(request: NextRequest) {
  try {
    ...
    const insertData = {
      ...
      medical_record_date_accomplished:
        formData.medicalRecord?.dateAccomplished || null,
      ...
      limited_intervention:
        formData.carePreferences?.limitedIntervention || false,
      iv_fluid:
        formData.carePreferences?.limitedInterventionOptions?.ivFluid || false,
      ng_tube:
        formData.carePreferences?.limitedInterventionOptions?.ngTube || false,
      gt_tube:
        formData.carePreferences?.limitedInterventionOptions?.gtTube || false,
      // o2_therapy: formData.carePreferences?.limitedInterventionOptions?.o2Therapy || false,
      cpap_bipap:
        formData.carePreferences?.limitedInterventionOptions?.cpapBipap ||
        false,
      antibiotics:
        formData.carePreferences?.limitedInterventionOptions?.antibiotics ||
        false,
      laboratory:
        formData.carePreferences?.limitedInterventionOptions?.laboratory ||
        false,
      diagnostics:
        formData.carePreferences?.limitedInterventionOptions?.diagnostics ||
        false,
      ...
      decision_maker_signature:
        formData.decisionMaker?.signatureImagePath ||
      ...
      physician_signature:
        formData.physician?.signatureImagePath ||
  ...
  try {
    ...
    if (id) {
      ...
      if (error) {
        ...
        return NextResponse.json(
          {
            message: 'Failed to fetch advance directive',
            error: error.message,
          },
          { status: 500 }
        );
      ...
      if (error) {
        ...
        return NextResponse.json(
          {
            message: 'Failed to fetch advance directives',
            error: error.message,
          },
          { status: 500 }
        );

...
export async function POST(request: NextRequest) {
  try {
    ...
    // Prepare the data for insertion - FIXED: Convert string boolean values to actual booleans
    const insertData = {
      ...
      // Mental Status Assessment - FIXED: Proper boolean conversion
      oriented_person_place_time: formData.oriented_person_place_time === true || formData.oriented_person_place_time === 'yes',
      coherent_speech: formData.coherent_speech === true || formData.coherent_speech === 'yes',
      hallucinations: formData.hallucinations === true || formData.hallucinations === 'yes',
      suicidal_homicidal_ideation: formData.suicidal_homicidal_ideation === true || formData.suicidal_homicidal_ideation === 'yes',
      understands_refusal_consequences: formData.understands_refusal_consequences === true || formData.understands_refusal_consequences === 'yes',
      
      // Narrative
      narrative_description: formData.narrative_description || null,
      
      // Refusal Options - FIXED: Proper boolean conversion
      refused_treatment_and_transport: formData.refused_treatment_and_transport === true,
      refused_treatment_willing_transport: formData.refused_treatment_willing_transport === true,
      wants_treatment_refused_transport: formData.wants_treatment_refused_transport === true,
      
      // Witness Information
      witness_name: formData.witness_name || null,
      witness_date: formData.witness_date || null,
      witness_signature_image: formData.witness_signature_image || null,
    ...
    // FIXED: Use the correct table name from your SQL migration
    const { data, error } = await supabase
      .from('conduction_refusal_forms') // Changed from 'conduction_refusal_form' to 'conduction_refusal_forms'
      .insert([insertData])
      .select();
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (id) {
      const { data, error } = await supabase
        .from('conduction_refusal_forms') // Fixed table name
        .select('*')
        .eq('id', id)
        .single();
      ...
      if (error) {
        ...
        return NextResponse.json(
          { message: 'Failed to fetch conduction refusal form', error: error.message },
          { status: 500 }
        );
      }
      ...
      if (error) {
        console.error('Supabase GET error:', error);
        return NextResponse.json(
          { message: 'Failed to fetch conduction refusal forms', error: error.message },
          { status: 500 }
        );
      }
